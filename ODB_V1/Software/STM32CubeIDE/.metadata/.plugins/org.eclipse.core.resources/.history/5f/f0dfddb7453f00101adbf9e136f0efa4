...
void SPI_Enable_Clock(SPI_TypeDef *SPIx) {
    if (SPIx == SPI1) {
        ...
        GPIO_InitPeriph(GPIOA, 2, OUT, O_PP, IO_NPP, GPIO_SPEED_3);		// CS RFM (A2)
        GPIO_InitPeriph(GPIOA, 4, OUT, O_PP, IO_NPP, GPIO_SPEED_3);		// CS ICM (A4)
        GPIO_InitPeriph(GPIOA, 5, ALT, AF5, IO_NPP, GPIO_SPEED_3);		// CLK (A5)
        GPIO_InitPeriph(GPIOA, 6, ALT, AF5, IO_PU, GPIO_SPEED_3);		// MISO (A6)
        GPIO_InitPeriph(GPIOA, 7, ALT, AF5, IO_NPP, GPIO_SPEED_3);		// MOSI (A7)

        GPIO_WritePin(GPIOA, 2, HIGH);
        GPIO_WritePin(GPIOA, 4, HIGH);
    } else if (SPIx == SPI4) {
        RCC->APB2ENR |= RCC_APB2ENR_SPI4EN;
        GPIO_InitPeriph(GPIOE, 4, OUT, O_PP, IO_NPP, GPIO_SPEED_3);		// CS BARO (E4)
        GPIO_InitPeriph(GPIOE, 12, ALT, AF5, IO_NPP, GPIO_SPEED_3);		// CLK (E12)
        GPIO_InitPeriph(GPIOE, 13, ALT, AF5, IO_PU, GPIO_SPEED_3);		// MISO (E13)
        GPIO_InitPeriph(GPIOE, 14, ALT, AF5, IO_NPP, GPIO_SPEED_3);		// MOSI (E14)
