...
class W25QParser:
    ...
    def scan_memory(self):
        ...
        # 1. Identifier les secteurs vides
        ...
        # 3. Extraire les vols (Header, Stats, Data)
        for s in range(sectors):
            addr = s * self.sector_size
            if addr + 4 > data_len: break
            
            magic = int.from_bytes(self.data[addr:addr+4], 'little')
            if magic == 0x48454144:  # HEAD
                ...
                # Vérifier si le secteur suivant est STAT
                ...
                # Identifier tous les secteurs de DATA pour ce vol
    ...
    def _parse_config(self, off):
        try:
            ...
            self.config_details = [
                ...
                ("Nom ODB", name),
                ("Rôle", f"{stage_role} (0=BOOSTER, 1=SUSTAINER)"),
    ...
    def get_flight_packets(self, flight):
        ...
        while off + 4 <= len(self.data):
            if int.from_bytes(self.data[off:off+4], 'little') != 0x44415441: break
            odb_off = off + 4
            if odb_off + 128 > len(self.data): break
