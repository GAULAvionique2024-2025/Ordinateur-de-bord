...
typedef struct __mavlink_rocket_telemetry_t {
 ...
 float altitude_agl_m; /*< [m] Altitude in meters from barometer referenced to ground level.*/
 float pressure_pa; /*< [Pa] Atmospheric pressure in pascals.*/
 ...
 float kalman_z; /*< [m] Filtered altitude from the Kalman filter in AGL.*/
 float kalman_v; /*< [m/s] Filtered velocity from the Kalman filter.*/
 uint16_t system_states; /*<  System component health flags.*/
 uint16_t event_states; /*<  Flight events such as pyros and apogee.*/
...
#define MAVLINK_MSG_ID_ROCKET_TELEMETRY_CRC 69
#define MAVLINK_MSG_ID_1350_CRC 69
...
#define MAVLINK_MESSAGE_INFO_ROCKET_TELEMETRY { \
    ...
    {  { "time_boot_ms", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_rocket_telemetry_t, time_boot_ms) }, \
         ...
         { "altitude_agl_m", NULL, MAVLINK_TYPE_FLOAT, 0, 52, offsetof(mavlink_rocket_telemetry_t, altitude_agl_m) }, \
 ...
 * @param altitude_agl_m [m] Altitude in meters from barometer referenced to ground level.
 * @param pressure_pa [Pa] Atmospheric pressure in pascals.
 ...
 * @param kalman_z [m] Filtered altitude from the Kalman filter in AGL.
                               ...
                               uint32_t time_boot_ms, uint16_t system_states, uint16_t event_states, uint8_t mission_state, uint16_t battery_mv, float roll, float pitch, float yaw, float imu_acc_x, float imu_acc_y, float imu_acc_z, float imu_gyro_x, float imu_gyro_y, float imu_gyro_z, float imu_mag_x, float imu_mag_y, float imu_mag_z, float altitude_agl_m, float pressure_pa, float temp_celsius, float highg_acc_x, float highg_acc_y, float highg_acc_z, uint8_t gps_fix, int32_t lat, int32_t lon, int32_t gps_alt, uint16_t vel, uint16_t cog, uint8_t satellites_nb, float imu_acc_vertical, float highg_acc_vertical, float kalman_z, float kalman_v)
{
    ...
    _mav_put_float(buf, 52, altitude_agl_m);
    ...
    packet.altitude_agl_m = altitude_agl_m;
...
/**
 * @brief Get field altitude_agl_m from rocket_telemetry message
 *
 * @return [m] Altitude in meters from barometer referenced to ground level.
 */
static inline float mavlink_msg_rocket_telemetry_get_altitude_agl_m(const mavlink_message_t* msg)
{
 ...
 *
 * @return [Pa] Atmospheric pressure in pascals.
 */
 ...
 *
 * @return [m] Filtered altitude from the Kalman filter in AGL.
 */
...
static inline void mavlink_msg_rocket_telemetry_decode(const mavlink_message_t* msg, mavlink_rocket_telemetry_t* rocket_telemetry)
{
    ...
    rocket_telemetry->altitude_agl_m = mavlink_msg_rocket_telemetry_get_altitude_agl_m(msg);
