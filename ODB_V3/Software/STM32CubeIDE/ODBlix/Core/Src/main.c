...
adxl382_t adxl382 = {
	...
	.filters_cfg = {
		.iir7_bypass  = false,
		.sinc_rate    = ADXL382_SINC_32X,
		.iir1_enable  = false,
		.round_mode   = ADXL382_ROUND_TRUNCATE,
		.dcf_bypass   = false,
		.eq_bypass    = true,
		.lpf_mode     = ADXL382_LPF_ODR_16,
		.hpf_path     = ADXL382_HPF_DSM,
		.hpf_corner   = ADXL382_HPF_NONE,
		.or_behavior  = ADXL382_OR_HOLD_PREV
	},
	.x_axis_offset = {
		.c0 = 0.0f,
		.c1 = 0.0f,
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : USB_5V_Pin */
  GPIO_InitStruct.Pin = USB_5V_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(USB_5V_GPIO_Port, &GPIO_InitStruct);
