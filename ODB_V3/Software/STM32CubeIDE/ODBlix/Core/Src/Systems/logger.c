...
static void Logger_ScanFlash(uint32_t *next_free_addr, uint32_t *next_id) {
    ...
    while(addr < LOGGER_MAX_ALLOWED_ADDRESS) {
        if(Logger_ReadHeader(addr, &header)) {
			...
			W25Q_Read(&w25q, (uint8_t*)&data_word, addr + W25Q512_PAGE_SIZE, 4);

			if(first_word != 0xFFFFFFFF || data_word != 0xFFFFFFFF) {
				addr += FLASH_SECTOR_SIZE_BYTE;
			} else {
				break;
			}
...
static void Logger_HeaderCSV() {
	...
	f_puts("TimeBoot_ms,Sys_States,Event_States,Mission_State,Battery_mV,", &active_file);
...
bool Logger_IsBusy(void) {
    return (flush_pending || logger_state != LOGGER_IDLE);
}
...
/* === TEST === */
/*
static void Logger_WriteStatsToCSV(const odb_stats_t *stats) {
    char header_buf[256];
    ...
    if (stats != NULL) {
        ...
        f_puts("# STATS CORRUPTED OR MISSING\n", &active_file);
    }
...
void Logger_DumpAllToSD(void) {
    ...
    while (addr < LOGGER_MAX_ALLOWED_ADDRESS) {
        if (Logger_ReadHeader(addr, &header)) {
            uint32_t flight_id = header.flight_id;
            snprintf(filename, sizeof(filename), "FLT_%04lu.CSV", flight_id);

            if (f_open(&active_file, filename, FA_WRITE | FA_CREATE_ALWAYS) == FR_OK) {

                uint32_t stats_addr = addr + FLASH_SECTOR_SIZE_BYTE;
                logger_stats_t stats_packet;
                bool has_stats = false;

                if(W25Q_Read(&w25q, (uint8_t*)&stats_packet, stats_addr, sizeof(logger_stats_t)) == 0) {
                    if (stats_packet.magic_number == LOGGER_STATS_MAGIC_NUMBER) {
                        has_stats = true;
                    }
                }

                if (has_stats) {
                    Logger_WriteStatsToCSV(&stats_packet.stats);
                } else {
                    Logger_WriteStatsToCSV(NULL);
                }
                ...
                if(data_count > 0) {
                    addr = ((cursor / FLASH_SECTOR_SIZE_BYTE) + 1) * FLASH_SECTOR_SIZE_BYTE;
                } else {
                    addr += FLASH_SECTOR_SIZE_BYTE;
                }
