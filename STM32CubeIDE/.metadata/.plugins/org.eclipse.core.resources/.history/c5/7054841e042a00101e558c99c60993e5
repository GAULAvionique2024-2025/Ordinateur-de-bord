...
static void MS5803_SendCommand(MS5803 *dev, uint8_t cmd) {
    CS_Select(dev);
    SPI_TransmitReceive(dev->SPIx, &cmd, NULL, 1, TIMEOUT);
    CS_Deselect(dev);
}
...
void MS5803_ReadTemperaturePressure(MS5803 *dev) {
    ...
    if(temp < 2000) {
    	...
    	offset2 = 0.0f;
    	sensibility2 = 0.0f;
    	if(temp >= 4500) sensibility2 -= ((temp - 4500) * (temp - 4500)) >> 3;
    }
    temp -= T2;
    offset -= offset2;
