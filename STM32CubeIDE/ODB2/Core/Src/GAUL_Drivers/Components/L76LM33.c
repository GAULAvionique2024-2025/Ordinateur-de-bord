...
int8_t L76LM33_Init(UART_HandleTypeDef *UARTx, L76LM33 *L76_data) {
    ...
    if (HAL_UART_Receive_IT(L76_data->UARTx, &(L76_data->received_byte), 1) != HAL_OK) {
...
void L76LM33_RxCallback(UART_HandleTypeDef *UARTx, L76LM33 *L76_data) {
    if (UARTx->Instance == L76_data->UARTx->Instance) {
        ...
        // Receive UART data with interrupts
        HAL_UART_Receive_IT(L76_data->UARTx, &(L76_data->received_byte), 1);
    }
}
...
int8_t L76LM33_Send_Command(L76LM33 *L76_data, char command[], uint8_t size) {
    ...
    if (HAL_UART_Transmit(L76_data->UARTx, (uint8_t *)command, size, L76LM33_UART_TIMEOUT) != HAL_OK) {
        return L76LM33_ERROR; // Error with UART
    }
